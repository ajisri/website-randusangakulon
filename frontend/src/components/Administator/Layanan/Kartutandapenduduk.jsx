...
const Kartutandapenduduk = () => {
  ...
  const [pname, setPname] = useState("kartutandapenduduk");
  const [kartutandapendudukContent, setKartutandapendudukContent] =
    useState("");
  ...
  const { data, error, isLoading } = useSWR(
    "http://localhost:5000/kartutandapenduduk",
    fetcher
  );
  ...
  useEffect(() => {
    if (data?.service) {
      ...
      setKartutandapendudukContent(data.service.content);
  ...
  const handleTextChange = useCallback((value) => {
    setKartutandapendudukContent(value);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("kartutandapendudukContent", kartutandapendudukContent);
    formData.append("status", status);

    try {
      const response = await axiosJWT.post(
        "http://localhost:5000/ckartutandapenduduk",
        formData,
        {
          headers: {
      ...
      await mutate("http://localhost:5000/kartutandapenduduk");
              ...
              <Card className="cart">
                ...
                <div className="p-field custom-editor">
                  ...
                  <div className="quill-wrapper">
                    <ReactQuill
                      value={kartutandapendudukContent}
